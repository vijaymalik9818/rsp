...
class AgentController extends Controller
{
    ...
    public function store_03_oct(Request $request)
    {
     ...
     public function store(Request $request)
    {
        ...
        try {
            ...
            if ($role == 2) {
                ...
                if ($request->hasFile('agent_profile')) {
                    $imageContent = file_get_contents($request->file('agent_profile')->getRealPath());
                    $s3Key1 = "agent-images-first/{$slug}_profile." . $request->agent_profile->extension();
                    ...
                    try {
                        ...
                        $profile_image = $result1['ObjectURL']; 
                ...
                if ($request->hasFile('agent_profile2')) {
                    $imageContent2 = file_get_contents($request->file('agent_profile2')->getRealPath());
                    $s3Key2 = "agent-images-first/{$slug}_profile2." . $request->agent_profile2->extension();
                    ...
                    try {
                        ...
                        $profile_image2 = $result2['ObjectURL']; 
                    } catch (AwsException $e) {
                        Log::error("AWS S3 Error: " . $e->getMessage());
                        return redirect()->back()->with('error', 'Failed to upload additional profile image.');
                    }
                } elseif (!empty($request->input('old_image2'))) {
                    $profile_image2 = $request->input('old_image2');
                }
                ...
                $userAttributes = [
                    ...
                    'other_profile_picture' =>  $profile_image2,
                    'password' =>  'realtor@123',
                ];
                ...
                if ($request->hasFile('agent_profile')) {
                    $imageContent = file_get_contents($request->file('agent_profile')->getRealPath());
                    $s3Key = "agent-images-first/{$slug}_profile." . $request->agent_profile->extension();
                    ...
                    try {
                        $result = $s3->putObject([
                            ...
                            'Key' => $s3Key,
                            'Body' => $imageContent
                            // 'ACL' => 'public-read', // Optional: Set ACL
                        ]);
                        $profile_image = $result['ObjectURL']; // Get the URL of the uploaded image
                    } catch (AwsException $e) {
                        Log::error("AWS S3 Error: " . $e->getMessage());
                        return redirect()->back()->with('error', 'Failed to upload profile image.');
                    }
                ...
                if ($request->hasFile('agent_logo')) {
                    $logoContent = file_get_contents($request->file('agent_logo')->getRealPath());
                    $s3LogoKey = "agent-images-first/{$slug}_logo." . $request->agent_logo->extension();
                    
                    try {
                        $resultLogo = $s3->putObject([
                            'Bucket' => env('AWS_BUCKET'),
                            'Key' => $s3LogoKey,
                            'Body' => $logoContent
                            // 'ACL' => 'public-read', // Optional: Set ACL
                        ]);
                        $logo_image = $resultLogo['ObjectURL']; // Get the URL of the uploaded logo
                    } catch (AwsException $e) {
                        Log::error("AWS S3 Error: " . $e->getMessage());
                        return redirect()->back()->with('error', 'Failed to upload logo image.');
                    }
                } elseif (!empty($request->input('old_logo'))) {
                    $logo_image = $request->input('old_logo');
                }
    ...
    public function moveAgentImagesToS3_bkup()
    {
        $agents = User::whereNotNull('profile_picture')->get();
        ...
        foreach ($agents as $agent) {
            ...
            $backendUrl = env('BACKEND_URL') . $profilePicture;
    
            $s3Key = "agent-images-first/{$agent->slug_url}.jpeg";
            ...
            try {
                ...
                $agent->profile_picture = $s3Url;
                $agent->agent_logo = $s3Url;
                $agent->save();
    
                echo "Successfully moved image for agent with MLS ID/Slug {$s3Key} to S3." . PHP_EOL;
    
            } catch (\Exception $e) {
                echo "Error processing image for agent with MLS ID/Slug {$s3Key}: " . $e->getMessage() . PHP_EOL;
            }
        }
    
        return response()->json(['status' => 'success', 'message' => 'Images moved to S3 successfully.']);
    }
...
if($status != 2){
     ...
     public function submitTicket(Request $request)
    {
        ...
        if ($request->hasFile('attachment')) {
            $imageContent = file_get_contents($request->file('attachment')->getRealPath());
            $s3Key1 = "ticket/attachment" . $request->attachment->extension();
...
// }
    ...
    public function fetchAndUploadImagesToS3()
    {

        $featuredPropertiesResponse = $this->getFeaturedProperty();
        if (!$featuredPropertiesResponse) {
            echo "No featured properties found.\n";
            return;
        }


        $data = json_decode($featuredPropertiesResponse->getContent(), true);
        if (!$data['success'] || !isset($data['properties'])) {
            echo "Failed to fetch properties from getFeaturedProperty.\n";
            return;
        }
        ...
        foreach ($properties as $property) {
            ...
            try {
                ...
                if ($imageResource === false) {
                    throw new \Exception("Failed to create image from string for $cityName");
                }
                ...
                imagecopyresampled($compressedImage, $imageResource, 0, 0, 0, 0, $newWidth, $newHeight, $originalWidth, $originalHeight);
                ...
                $cityNameSanitized = str_replace(' ', '_', strtolower($cityName));
                $s3Key = "property-city-first/{$cityNameSanitized}/photo-{$cityNameSanitized}.jpeg";


                $result = $s3->putObject([
                    'Bucket' => env('AWS_BUCKET'),
                    'Key' => $s3Key,
                    'Body' => $compressedImageContent,
                ...
                CityProperty::where('city_name', $cityName)->update(['image' => $result['ObjectURL']]);


                echo "Changed Image of $cityName\n";

            } catch (\Exception $e) {

                echo "Failed to upload image for city: $cityName - Error: " . $e->getMessage() . "\n";
            }
        }
    }
    ...
    public function getsearchautoquerytour(Request $request)
{
      ...
      public function personalProfileEdit(Request $request)
    {
        ...
        $profile_image = "";
        $adminname = $request->input('agent_first') . ' ' . $request->input('agent_last');
        $adminNameSanitized = str_replace(' ', '_', strtolower($adminname));
        if ($request->hasFile('agent_profile')) {
            $imageContent = file_get_contents($request->file('agent_profile')->getRealPath());
                    $s3Key1 = "admin-images-first/{$adminNameSanitized}_profile." . $request->agent_profile->extension();
